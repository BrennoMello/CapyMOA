...
class DERPP(BatchClassifier, TrainTaskAware, TestTaskAware):
    #TODO: Change the docstring to reflect the new name of the class
    """Continual learning via Dark Experience Replay++.

    Dark Experience Replay (DERPP) [#f0]_ is a replay continual learning
    ...
    * Not :class:`~capymoa.ocl.base.TrainTaskAware` or
    ...
    def train_step(self, x_fresh: Tensor, y_fresh: Tensor) -> None:
        ...
        outputs_logits = self.learner.predict_logits(x_fresh)
        ...
        if self.coreset.count != 0:
            ...
            x_replay, y_replay, logits_replay = self.coreset.sample(n)
            outputs_logits_replay = self.learner.predict_logits(x_replay)
            y_replay = y_replay.to(self.learner.device, self.learner.y_dtype)
            loss_ce = self.beta * self.learner.criterion(outputs_logits_replay, y_replay)
            loss_fresh += loss_ce
